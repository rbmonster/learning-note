...
### 表中ABC三个字段创建联合索引，那些情况可以走到该索引？
- `where A = 10 `: 可以走索引，最左原则
- `where B = 10 `: 不走索引
- `where A = 10 and C =3 `: 可以走索引，但是效果差一些
-` where B= 7 and A = 6 `: 优化器会进行过滤条件优化变成A先过滤再过滤B，可以走索引
- `where A in (1,2,3)`: 可以走索引，因为A覆盖了一个范围，但是如果覆盖的范围过大，或者数据量过大都可能导致索引失效
- `where A > 8 and A < 100`: 同上
- `where A like '%2%'`: 不满足最左原则
- `where A like '2%'`: 满足最左原则
- `where A*3 = 90`: 不走索引
...
### innodb引擎索引结构？B+树与B树的区别？
见上文索引结构，回答思路：
1. MySQL索引的数据结构使用的是B+树
2. B+树对比B-树的特点
3. MySQL的聚簇索引与非聚簇索引的区别，非聚簇索引叶子节点存储的是主键ID

### 可重复读隔离级别是否解决了幻读问题？
**只是MySQL的innoDB的**可重复读隔离级别使用多版本并发控制解决了幻读问题\
正常的数据库在可重复读隔离级别，只是解决了数据可重复读问题，而不能避免幻读问题。
