...
&emsp;&emsp;&emsp;<a href="#64">5.5.1. 为何需要两个日志</a>  
&emsp;&emsp;<a href="#65">5.6. 两种日志区别</a>  
&emsp;&emsp;<a href="#66">5.7. 两阶段提交的组提交机制执行流程</a>  
&emsp;<a href="#67">6. 实际sql的执行</a>  
&emsp;&emsp;<a href="#68">6.1. count(*)实现</a>  
&emsp;&emsp;<a href="#69">6.2. order by 处理流程</a>  
&emsp;&emsp;&emsp;<a href="#70">6.2.1. 全字段排序</a>  
&emsp;&emsp;&emsp;<a href="#71">6.2.2. rowId排序</a>  
&emsp;&emsp;<a href="#72">6.3. join的执行过程</a>  
&emsp;&emsp;&emsp;<a href="#73">6.3.1. Index Nested-Loop Join</a>  
&emsp;&emsp;&emsp;<a href="#74">6.3.2. Simple Nested-Loop Join</a>  
&emsp;&emsp;&emsp;<a href="#75">6.3.3. Block Nested-Loop Join</a>  
&emsp;&emsp;&emsp;<a href="#76">6.3.4. join语句mysql的优化</a>  
&emsp;&emsp;&emsp;<a href="#77">6.3.5. 总结</a>  
&emsp;&emsp;<a href="#78">6.4. union执行流程</a>  
&emsp;&emsp;&emsp;<a href="#79">6.4.1. union all</a>  
&emsp;&emsp;<a href="#80">6.5. group by 执行流程</a>  
&emsp;&emsp;&emsp;<a href="#81">6.5.1. group by 优化 —— 索引</a>  
&emsp;&emsp;&emsp;<a href="#82">6.5.2. group by 优化 —— 直接排序</a>  
&emsp;&emsp;&emsp;<a href="#83">6.5.3. 小结</a>  
&emsp;<a href="#84">7. MySQL中的组件</a>  
&emsp;&emsp;<a href="#85">7.1. Buffer Pool</a>  
&emsp;&emsp;&emsp;<a href="#86">7.1.1. 内存管理策略</a>  
&emsp;&emsp;&emsp;<a href="#87">7.1.2. change buffer</a>  
&emsp;&emsp;&emsp;<a href="#88">7.1.3. change buffer 和 redo log</a>  
&emsp;&emsp;<a href="#89">7.2. sort buffer</a>  
&emsp;&emsp;<a href="#90">7.3. 内存临时表</a>  
&emsp;&emsp;<a href="#91">7.4. 磁盘临时表</a>  
&emsp;&emsp;<a href="#92">7.5. binlog cache</a>  
&emsp;&emsp;<a href="#93">7.6. redo log buffer</a>  
&emsp;&emsp;<a href="#94">7.7. net_buffer</a>  
&emsp;&emsp;<a href="#95">7.8. join buffer</a>  
&emsp;<a href="#96">8. 数据库设计</a>  
&emsp;&emsp;<a href="#97">8.1. 数据库设计原则</a>  
&emsp;&emsp;&emsp;<a href="#98">8.1.1. 第一范式(列不可再分)</a>  
&emsp;&emsp;&emsp;<a href="#99">8.1.2. 第二范式(确保表中的每列都和主键相关)</a>  
&emsp;&emsp;&emsp;<a href="#100">8.1.3. 第三范式(确保每列都和主键列直接相关,而不是间接相关)</a>  
&emsp;&emsp;&emsp;<a href="#101">8.1.4. 相关资料</a>  
&emsp;&emsp;<a href="#102">8.2. 表字段设计</a>  
&emsp;&emsp;&emsp;<a href="#103">8.2.1. 乐观锁字段</a>  
&emsp;&emsp;&emsp;<a href="#104">8.2.2. 通用字段</a>  
&emsp;&emsp;&emsp;<a href="#105">8.2.3. 日期字段的选择</a>  
&emsp;&emsp;<a href="#106">8.3. 实际设计问题 —— 设计部门表 </a>  
&emsp;<a href="#107">9. MySQL架构</a>  
&emsp;&emsp;<a href="#108">9.1. 主从模式(读写分离)</a>  
&emsp;&emsp;<a href="#109">9.2. 分库分表</a>  
&emsp;&emsp;&emsp;<a href="#110">9.2.1. 分库分表场景</a>  
&emsp;&emsp;&emsp;<a href="#111">9.2.2. 垂直切分</a>  
&emsp;&emsp;&emsp;<a href="#112">9.2.3. 水平切分</a>  
&emsp;&emsp;&emsp;<a href="#113">9.2.4. 分库分表的查询</a>  
&emsp;&emsp;<a href="#114">9.3. MGR(MySQL Group Replication)</a>  
&emsp;&emsp;<a href="#115">9.4. 主从数据同步</a>  
&emsp;&emsp;&emsp;<a href="#116">9.4.1. MySQL异步复制</a>  
&emsp;&emsp;&emsp;<a href="#117">9.4.2. MySQL半同步复制</a>  
&emsp;&emsp;&emsp;<a href="#118">9.4.3. MySQL组复制</a>  
&emsp;<a href="#119">10. 其他面试题</a>  
&emsp;&emsp;<a href="#120">10.1. 主键索引是否使用自增</a>  
&emsp;&emsp;<a href="#121">10.2. 临时表与内存表</a>  
&emsp;&emsp;<a href="#122">10.3. 重建索引是否合理</a>  
&emsp;&emsp;<a href="#123">10.4. mysql数据库抖动</a>  
&emsp;&emsp;<a href="#124">10.5. 读已提交和可重复读是如何实现的</a>  
&emsp;&emsp;<a href="#125">10.6. 读已提交和可重复读区别？</a>  
&emsp;&emsp;<a href="#126">10.7. 数据库数据库一致性是如何实现的？</a>  
&emsp;&emsp;<a href="#127">10.8. redoLog、undoLog、binlog区别？</a>  
&emsp;&emsp;&emsp;<a href="#128">10.8.1. 两种日志有以下三点不同</a>  
&emsp;&emsp;<a href="#129">10.9. 一个事务开启什么时候产生三种日志？</a>  
&emsp;&emsp;<a href="#130">10.10. 可重复读隔离级别下，事务中select一条记录巨慢</a>  
&emsp;&emsp;<a href="#131">10.11. 数据库什么情况会出现死锁？如何处理死锁？</a>  
&emsp;&emsp;&emsp;<a href="#132">10.11.1. 由于索引导致的死锁</a>  
&emsp;&emsp;<a href="#133">10.12. 如何优化SQL</a>  
&emsp;&emsp;&emsp;<a href="#134">10.12.1. order by 优化</a>  
&emsp;&emsp;&emsp;<a href="#135">10.12.2. Subquery 子查询</a>  
&emsp;&emsp;&emsp;<a href="#136">10.12.3. limit 优化 - 延迟关联</a>  
&emsp;&emsp;&emsp;&emsp;<a href="#137">10.12.3.1. 相关资料</a>  
&emsp;&emsp;&emsp;<a href="#138">10.12.4. Or/And Condition</a>  
&emsp;&emsp;&emsp;<a href="#139">10.12.5. join 优化</a>  
&emsp;&emsp;<a href="#140">10.13. 数据库连接池、数据库连接线程安全的吗？</a>  
&emsp;&emsp;<a href="#141">10.14. InnoDB 和 MyIsam 数据库引擎的区别</a>  
&emsp;&emsp;<a href="#142">10.15. MySQL数据页(储存页)</a>  
&emsp;&emsp;<a href="#143">10.16. 高并发数据库读写压力大怎么处理？</a>  
&emsp;&emsp;<a href="#144">10.17. 其他</a>  
&emsp;&emsp;&emsp;<a href="#145">10.17.1. 什么是覆盖索引跟回表？</a>  
&emsp;&emsp;&emsp;<a href="#146">10.17.2. `left join,right join,inner join`,表表关联什么区别？</a>  
&emsp;&emsp;&emsp;<a href="#147">10.17.3. 大事务会有什么影响？</a>  
...
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/learning/basic/mvcc-line.png)
...
例子：
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/versionChain.png)
...
#### <a name="64">为何需要两个日志</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="65">两种日志区别</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="66">两阶段提交的组提交机制执行流程</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
## <a name="67">实际sql的执行</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
一个sql 语句mysql的执行顺序：
...
### <a name="68">count(*)实现</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
不同引擎中的实现
...
### <a name="69">order by 处理流程</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
#### <a name="70">全字段排序</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
```
CREATE TABLE `t` (
`id` int(11) NOT NULL,
...
#### <a name="71">rowId排序</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="72">join的执行过程</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
以下为三种join算法，主要的区别在于连接的字段是否有索引
#### <a name="73">Index Nested-Loop Join</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/indexJoin.jpg)
...
#### <a name="74">Simple Nested-Loop Join</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
```
...
#### <a name="75">Block Nested-Loop Join</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
执行过程：
...
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/blockJoin.jpg)


#### <a name="76">join语句mysql的优化</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
Multi-Range Read优化
...
#### <a name="77">总结</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="78">union执行流程</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/union1.png)
...
#### <a name="79">union all</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
```
...
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/groupby.png)
...
```
#### <a name="81">group by 优化 —— 索引</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
索引保证了数据有序，在group by时候，分组计数计算时一片区域的id都是连续的，整个表扫描结束时便可以拿到结果，不需要临时表也不需要排序。

#### <a name="82">group by 优化 —— 直接排序</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/lru.png)
...
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/changebuffer.png)
...
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/orderTmpMemoryTable.png)
...
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/readwrite1.png)
...
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/readwrite2.png)
...
![image](https://gitee.com/rbmon/file-storage/blob/main/learning-note/other/mysql/shardTable.png)
...
#### <a name="135">Subquery 子查询</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
对于子查询，一般的优化策略是做等价改写\
Subquery优化总结：\
1. 子查询会用到临时表，需尽量避免；
