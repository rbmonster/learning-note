...
&emsp;&emsp;&emsp;<a href="#60">5.3.1. bin log写入机制</a>  
&emsp;&emsp;<a href="#61">5.4. 两种日志区别</a>  
&emsp;&emsp;<a href="#62">5.5. 两阶段提交</a>  
&emsp;&emsp;&emsp;<a href="#63">5.5.1. 为何需要两个日志</a>  
&emsp;&emsp;&emsp;<a href="#64">5.5.2. 两阶段提交的实际执行流程</a>  
&emsp;&emsp;&emsp;<a href="#65">5.5.3. 其他</a>  
...
&emsp;&emsp;&emsp;<a href="#78">6.4.1. union all</a>  
&emsp;&emsp;<a href="#79">6.5. group by 执行流程</a>  
&emsp;&emsp;&emsp;<a href="#80">6.5.1. group by 优化 ——索引</a>  
&emsp;&emsp;&emsp;<a href="#81">6.5.2. group by 优化 —— 直接排序</a>  
&emsp;<a href="#82">7. MySQL中的buffer</a>  
&emsp;&emsp;<a href="#83">7.1. change buffer</a>  
&emsp;&emsp;&emsp;<a href="#84">7.1.1. change buffer 和 redo log</a>  
&emsp;&emsp;<a href="#85">7.2. sort buffer</a>  
&emsp;&emsp;<a href="#86">7.3. 内存临时表</a>  
&emsp;&emsp;<a href="#87">7.4. 磁盘临时表</a>  
&emsp;<a href="#88">8. 数据库设计</a>  
&emsp;&emsp;<a href="#89">8.1. 数据库设计原则</a>  
&emsp;&emsp;&emsp;<a href="#90">8.1.1. 第一范式(列不可再分)</a>  
&emsp;&emsp;&emsp;<a href="#91">8.1.2. 第二范式(确保表中的每列都和主键相关)</a>  
&emsp;&emsp;&emsp;<a href="#92">8.1.3. 第三范式(确保每列都和主键列直接相关,而不是间接相关)</a>  
&emsp;&emsp;&emsp;<a href="#93">8.1.4. 相关资料</a>  
&emsp;&emsp;<a href="#94">8.2. 表字段设计</a>  
&emsp;&emsp;&emsp;<a href="#95">8.2.1. 乐观锁字段</a>  
&emsp;&emsp;&emsp;<a href="#96">8.2.2. 通用字段</a>  
&emsp;&emsp;&emsp;<a href="#97">8.2.3. 日期字段的选择</a>  
&emsp;&emsp;<a href="#98">8.3. 实际设计问题 —— 设计部门表 </a>  
&emsp;<a href="#99">9. 分库分表</a>  
&emsp;&emsp;<a href="#100">9.1. 分库分表有了解吗？</a>  
&emsp;&emsp;<a href="#101">9.2. 垂直切分</a>  
&emsp;&emsp;<a href="#102">9.3. 水平切分</a>  
&emsp;<a href="#103">10. MGR(MySQL Group Replication)</a>  
&emsp;&emsp;<a href="#104">10.1. 主从数据同步</a>  
&emsp;&emsp;&emsp;<a href="#105">10.1.1. MySQL异步复制</a>  
&emsp;&emsp;&emsp;<a href="#106">10.1.2. MySQL半同步复制</a>  
&emsp;&emsp;&emsp;<a href="#107">10.1.3. MySQL组复制</a>  
&emsp;<a href="#108">11. 其他面试题</a>  
&emsp;&emsp;<a href="#109">11.1. 主键索引是否使用自增</a>  
&emsp;&emsp;<a href="#110">11.2. 重建索引是否合理</a>  
&emsp;&emsp;<a href="#111">11.3. mysql数据库抖动</a>  
&emsp;&emsp;<a href="#112">11.4. 读已提交和可重复读是如何实现的</a>  
&emsp;&emsp;<a href="#113">11.5. 读已提交和可重复读区别？</a>  
&emsp;&emsp;<a href="#114">11.6. 数据库数据库一致性是如何实现的？</a>  
&emsp;&emsp;<a href="#115">11.7. redoLog、undoLog、binlog区别？</a>  
&emsp;&emsp;&emsp;<a href="#116">11.7.1. 两种日志有以下三点不同</a>  
&emsp;&emsp;<a href="#117">11.8. 一个事务开启什么时候产生三种日志？</a>  
&emsp;&emsp;<a href="#118">11.9. 可重复读隔离级别下，事务中select一条记录巨慢</a>  
&emsp;&emsp;<a href="#119">11.10. 数据库什么情况会出现死锁？如何处理死锁？</a>  
&emsp;&emsp;&emsp;<a href="#120">11.10.1. 由于索引导致的死锁</a>  
&emsp;&emsp;<a href="#121">11.11. 如何优化SQL</a>  
&emsp;&emsp;&emsp;<a href="#122">11.11.1. order by 优化</a>  
&emsp;&emsp;&emsp;<a href="#123">11.11.2. Subquery 子查询</a>  
&emsp;&emsp;&emsp;<a href="#124">11.11.3. limit 优化 - 延迟关联</a>  
&emsp;&emsp;&emsp;&emsp;<a href="#125">11.11.3.1. 相关资料</a>  
&emsp;&emsp;&emsp;<a href="#126">11.11.4. Or/And Condition</a>  
&emsp;&emsp;&emsp;<a href="#127">11.11.5. join 优化</a>  
&emsp;&emsp;<a href="#128">11.12. 数据库连接池、数据库连接线程安全的吗？</a>  
&emsp;&emsp;<a href="#129">11.13. InnoDB 和 MyIsam 数据库引擎的区别</a>  
&emsp;&emsp;<a href="#130">11.14. MySQL数据页(储存页)</a>  
&emsp;&emsp;<a href="#131">11.15. 高并发数据库读写压力大怎么处理？</a>  
&emsp;&emsp;<a href="#132">11.16. 其他</a>  
...
#### <a name="60">bin log写入机制</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="61">两种日志区别</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="62">两阶段提交</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="63">为何需要两个日志</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="64">两阶段提交的实际执行流程</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
![image](https://github.com/rbmonster/file-storage/blob/main/learning-note/other/mysql/actualWrite.jpg)
WAL机制主要得益于两个方面：
1. redo log 和 binlog都是顺序写， 磁盘的顺序写比随机写速度要快；
2. 组提交机制， 可以大幅度降低磁盘的IOPS消耗。

#### <a name="65">其他</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="78">union all</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
```
(select 1000 as f) union all(select id from t1 order by id desc limit 2);
```
- 与上面的区别为union all不需要除重，因此直接把查询结果放在结果集中返回。

### <a name="79">group by 执行流程</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
```
...
```
#### <a name="80">group by 优化 ——索引</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
索引保证了数据有序，在group by时候，分组计数计算时一片区域的id都是连续的，整个表扫描结束时便可以拿到结果，不需要临时表也不需要排序。

#### <a name="81">group by 优化 —— 直接排序</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
确保数据量确实超过了sort buffer，可以直接强制mysql直接使用磁盘文件排序。
```
select SQL_BIG_RESULT id%100 as m, count(*) as c from t1 group by m;
```

## <a name="82">MySQL中的buffer</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

### <a name="83">change buffer</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="84">change buffer 和 redo log</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
```
...
### <a name="85">sort buffer</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
sort_buffer_size: 是MySQL为排序开辟的内存（`sort_buffer`） 的大小。 `如果要排序的数据量小于sort_buffer_size`， 排序就在内存中完成。 但如果排序数据量太大内存放不下， 则不得不利用磁盘临时文件辅助排序。
> sort_buffer_size大于了需要排序的数据量的大小， number_of_tmp_files就是0，排序直接在内存完成。

具体可见order by排序章节

### <a name="86">内存临时表</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="87">磁盘临时表</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
## <a name="88">数据库设计</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

### <a name="89">数据库设计原则</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

#### <a name="90">第一范式(列不可再分)</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="91">第二范式(确保表中的每列都和主键相关)</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
第二范式需要确保数据库表中的每一列都和主键相关，而不能只与主键的某一部分相关（主要针对联合主键而言）。也就是说在一个数据库表中，一个表中只能保存一种数据，不可以把多种数据保存在同一张数据库表中。
> 如一个订单的商品明细表，不应该订单主表的ID放到商品明细表中，会导致数据冗余

#### <a name="92">第三范式(确保每列都和主键列直接相关,而不是间接相关)</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
第三范式需要确保数据表中的每一列数据都和主键直接相关，而不能间接相关。
> 如一个订单表都会有客户字段，可以把客户编码作为一个外键跟订单表建立响应关系。

#### <a name="93">相关资料</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
- https://www.cnblogs.com/linjiqin/archive/2012/04/01/2428695.html

### <a name="94">表字段设计</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="95">乐观锁字段</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
使用version作为乐观锁的控制。

#### <a name="96">通用字段</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
创建人、创建时间、修改人、修改时间

#### <a name="97">日期字段的选择</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="98">实际设计问题 —— 设计部门表 </a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
设计一张部门表应该有哪些必要字段？
...
## <a name="99">分库分表</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

### <a name="100">分库分表有了解吗？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
由于数据量过大而导致数据库性能降低的问题，将原来独立的数据库拆分成若干数据库组成 ，将数据大表拆分成若干数据表组成，使得单一数据库、单一数据表的数据量变小，从而达到提升数据库性能的目的。

### <a name="101">垂直切分</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="102">水平切分</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
## <a name="103">MGR(MySQL Group Replication)</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="104">主从数据同步</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

参考资料：[MySQL5.7新特性--官方高可用方案MGR介绍](https://www.cnblogs.com/luoahong/articles/8043035.html)
#### <a name="105">MySQL异步复制</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="106">MySQL半同步复制</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
在master事务的commit之前，必须确保一个slave收到relay log并且响应给master以后，才能进行事务的commit。但是slave对于relay log的应用仍然是异步进行的

#### <a name="107">MySQL组复制</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
基于传统异步复制和半同步复制的缺陷——数据的一致性问题无法保证


## <a name="108">其他面试题</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

### <a name="109">主键索引是否使用自增</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
使用自增好处：
...
### <a name="110">重建索引是否合理</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="111">mysql数据库抖动</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="112">读已提交和可重复读是如何实现的</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
1. ReadView：
...
### <a name="113">读已提交和可重复读区别？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
- 举两个事务线程的例子。
     
### <a name="114">数据库数据库一致性是如何实现的？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
- 通过redoLog 与binlog 两阶段提交 保证事务一致性。

### <a name="115">redoLog、undoLog、binlog区别？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="116">两种日志有以下三点不同</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="117">一个事务开启什么时候产生三种日志？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
1. 事务开启
...
### <a name="118">可重复读隔离级别下，事务中select一条记录巨慢</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
session A：
```
start transaction;
// 时刻A
...
### <a name="119">数据库什么情况会出现死锁？如何处理死锁？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="120">由于索引导致的死锁</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="121">如何优化SQL</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="122">order by 优化</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
Order by查询的两种情况：
...
#### <a name="123">Subquery 子查询</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
对于子查询，一般的优化策略是做等价改写\
Subquery优化总结：\
1. 子查询会用到临时表，需尽量避免；
...
#### <a name="124">limit 优化 - 延迟关联</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
##### <a name="125">相关资料</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
#### <a name="126">Or/And Condition</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
处理策略
1. And子句多个条件中拥有一个过滤性较高的索引即可;
2. Or条件前后字段均要创建索引;
3. 为最常用的And组合条件创建复合索引。

#### <a name="127">join 优化</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="128">数据库连接池、数据库连接线程安全的吗？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="129">InnoDB 和 MyIsam 数据库引擎的区别</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="130">MySQL数据页(储存页)</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="131">高并发数据库读写压力大怎么处理？</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
### <a name="132">其他</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
- 什么是覆盖索引跟回表？
见索引规则
